...
- The optional viewer is NOT started from this container and must be run externally.
...
- A convenience env file db_visualizer_external/postgres.env is created for optional external tools.
- Guard: This container must never invoke `node server.js` or `npm start`. Ensure Dockerfile CMD/ENTRYPOINT only runs Postgres (via startup.sh). The script includes explicit guards to prevent Node/npm execution.

Optional tools (External Only):
- The db_visualizer_external directory contains an optional Node-based viewer for local debugging. Do not run it in this container.
- To use it (on host or separate utility container):
  1) cd skill_story_database/db_visualizer_external
  2) npm install
